resource "aws_autoscaling_group" "asg" {
  name_prefix               = "fake-service-${var.name}-${var.consul_datacenter}"
  max_size                  = 3
  min_size                  = 1
  ...
  tag {
    key                 = "Name"
    value               = "fake-service-${var.name}-${var.consul_datacenter}"
...
resource "aws_launch_template" "lt" {
  ...
  tag_specifications {
    ...
    tags = {
      Name = "fake-service-${var.name}-${var.consul_datacenter}",
